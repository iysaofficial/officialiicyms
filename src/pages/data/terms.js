export const indonesiaOnlineTerms = (
  <>
    <h5>
      Before proceeding, please read and agree to the following terms & conditions
      for Online Participants:
    </h5>
      <li>
        Please be advised that all data submitted by participants cannot be modified after the payment deadline has passed. In this regard participants are required to carefully review their Letter of Acceptance (LoA) and registration information upon submission to ensure the accuracy, completeness, and correctness of all details provided.
      </li>
      <li>
        Participants who do not submit the required documents (<em>full paper</em>, 
        <em> PowerPoint</em>) after two reminders will be considered to have withdrawn 
        automatically.
      </li>
      <li>
        Participants must use a title and <em>full paper</em> that corresponds to 
        the category they are participating in. (International participants must use English for 
        all required documents)
      </li>
      <li>
        The committee's decision regarding this matter is final and cannot be
        contested.
      </li>
      <br/>
    <p className="text-align">
      In addition, we would like to inform you that every event will be registered for the
      SIM T Puspresnas curation.
    </p>
    <p className="text-align">
      However, the curation results are entirely the authority of Puspresnas and cannot
      be contested by any party. Therefore, we do not
      accept any objections or complaints regarding the curation results that have
      been determined by Puspresnas.
    </p>
  </>
);


export const indonesiaOfflineTerms = (
  <>
    <h5>
      Before proceeding, please read and agree to the following terms & conditions
      for Offline Participants:
    </h5>
      <li>
        Please be advised that all data submitted by participants cannot be modified after the payment deadline has passed. In this regard participants are required to carefully review their Letter of Acceptance (LoA) and registration information upon submission to ensure the accuracy, completeness, and correctness of all details provided.
      </li>
      <li>
        Participants who do not submit the required documents (<em>full paper</em>,
        <em> poster</em>) after two reminders will be considered to have withdrawn
        automatically.
      </li>
      <li>
        Participants must use a title and <em>full paper</em> that corresponds to
        the category they are participating in. (International participants must use
        English for all <em>requirements</em>)
      </li>
      <li>
        Participants are required to make an A0 size poster, bring the product,
        and also a <em>paper</em>/<em>full paper</em> (in <em>hard copy</em>) during the judging
        session.
      </li>
      <li>
        The awarding of prizes to participants will be carried out according to the order
        that has been determined in the event schedule (participants are not allowed
        to ask for their <em>award</em> before their turn).
      </li>
      <li>
        Participants are required to take part in the entire series of activities in accordance
        with the schedule that has been prepared by the organizing committee.
      </li>
      <li>
        For offline participants who are entitled to the <em>Best Booth</em>, <em>Best Poster</em>,
        <em> Best Presentation</em>, <em>Best Project</em>, cash, <em>Semi Grand Award</em>, and 
        <em>Grand Award</em> prizes but are unable to attend the award ceremony, the prize
        in question will be forfeited.
      </li>
      <li>
        The committee's decision regarding this matter is final and cannot be
        contested.
      </li>
      <br/>
    <p className="text-align">
      In addition, we would like to inform you that every event will be registered for the
      SIM T Puspresnas curation.
    </p>
    <p className="text-align">
      However, the curation results are entirely the authority of Puspresnas and cannot
      be contested by any party. Therefore, we do not
      accept any objections or complaints regarding the curation results that have
      been determined by Puspresnas.
    </p>
  </>
);

export const internationalOnlineTerms = (
  <>
    <h3 className="text-2xl">Online Participants:</h3>
    <li>
      Please be advised that all data submitted by participants cannot be modified after the payment deadline has passed. In this regard participants are required to carefully review their Letter of Acceptance (LoA) and registration information upon submission to ensure the accuracy, completeness, and correctness of all details provided.
    </li>
    <li>
      Participants who do not submit the required documents (full paper,
      PowerPoint) after two reminders will be considered to have resigned
      automatically.
    </li>
    <li>
      Participants must use the title and full paper in accordance with
      the category they are participating in. (International participants must
      use English for all required documents)
    </li>
    <li>
      The committee's decision regarding this matter is final and cannot be
      contested.
    </li>
  </>
);

export const internationalOfflineTerms = (
  <>
    <h3 className="text-2xl">Offline Participants:</h3>
    <li>
      Please be advised that all data submitted by participants cannot be modified after the payment deadline has passed. In this regard participants are required to carefully review their Letter of Acceptance (LoA) and registration information upon submission to ensure the accuracy, completeness, and correctness of all details provided.
    </li>
    <li>
      Participants who do not submit the required files (full paper,
      poster) after two reminders will be considered automatically resigned.
    </li>
    <li>
      Participants must use titles and full papers that are in accordance
      with the category they are participating in. (International participants
      must use English for all requirements)
    </li>
    <li>
      Participants are required to make an A0-sized poster, bring the product
      and also the paper/full paper (in hard copy) during the judging
      session.
    </li>
    <li>
      The awarding of awards to participants will be carried out in accordance
      with the order set out in the schedule of events (participants are not
      allowed to request awards before their turn).
    </li>
    <li>
      Participants are required to follow the entire series of activities in
      accordance with the schedule prepared by the organizing committee.
    </li>
    <li>
      For offline participants who are entitled to the Best booth, Best Poster,
      Best Presentation, Best Project, cash, semi grand award, and grand award
      prizes but cannot attend the prize distribution ceremony, the prizes
      concerned will be declared forfeited.
    </li>
    <li>
      The committee's decision regarding this matter is final and cannot be
      contested.
    </li>
  </>
);
